...
var addWare = function(b, g, f, c, j, d) {
    if (isNotBlank(f) && f) {
        ...
        m.setAttribute("style", "position:absolute;left:0px;top:0px;background-color:rgb(20, 20, 20);filter:alpha(opacity=60);opacity: 0.6;width:" + a + ";height:" + k + ";z-index:9998;");
        (document.body || document.documentElement).appendChild(m);
        var i = "cart/add.json@wareId=" + b + "&num=" + g + (isNotBlank(f) && f ? "&isAjax=" + f : "") + "&resourceType=" + c + "&resourceValue=" + j + (isNotBlank(d) && (d != "") ? "&sid=" + d : "");
        var l = ajax(i, function(o) {
            e.stop();
            $("#spinner2").hide();
            var q = 100;
